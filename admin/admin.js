...
function buildGalleryEditor(containerId, items) {
  ...
  items.forEach((item, idx) => {
    container.appendChild(createGalleryCard(item, idx));
  });
  ...
  addBtn.onclick = () => {
    const card = createGalleryCard({}, container.children.length);
      ...
      <div class="grid md:grid-cols-3 gap-4">
        ...
        <div class="md:col-span-2 space-y-3">
          ...
          <div class="space-y-1.5">
            ...
            <select name="gallery-category-${idx}" class="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all">
              <option value="Teams" ${item.category === 'Teams' ? 'selected' : ''}>Teams</option>
...
function hydrateAdminForms() {
  ...
  const joinStreams = (joinPage.streams || []).map((stream) => ({
    ...stream,
    linksText: formatLinksText(stream.links)
  }));
  buildListEditor("join-streams-list", joinStreams, [
    ...
    { name: "badge", label: "Badge" },
    { name: "description", label: "Description", multiline: true },
    { name: "linksText", label: "Links (Label | URL)", multiline: true }
  ]);
  ...
  saveBtn?.addEventListener("click", async () => {
    ...
    joinPage.streams = joinStreamsRaw
      .map((stream) => ({
        ...
        links: parseLinksText(stream.linksText || "")
      }))
      .filter((stream) => stream.title || stream.description || (stream.links && stream.links.length));
  ...
  importInput?.addEventListener("change", (e) => {
    ...
    reader.onload = () => {
      try {
        ...
        saveSiteData(parsed);
        hydrateAdminForms();
        showToast(toast, "Imported data applied.");
      } catch (err) {
